...
SAIGA was developed for and accompanies the paper: [Seah, A.; Lim, M.C.; McAloose, D.; Prost, S.; Seimon, T.A. *MinION-Based DNA Barcoding of Preserved and Non-Invasively Collected Wildlife Samples*. Genes 2020, 11, 445](https://www.mdpi.com/2073-4425/11/4/445).
...
- *Create sample list text file, save it in the 2a_samp_lists folder.* The file must be named with `[year][month][day]_sample_list.txt`, where the date info is the day of the sequencing run (you could name the section in `[]` whatever you want, but SAIGA scripts use this label to keep track of files from different sequence runs). The file is tab-delimited. It requires the sample name, barcoding gene, amplicon length, and index (ONT or custom). One line per sample.
...
- `_spID/` contains outputs from Medaka and Blast search for eaech sample:
  - intermediate files generated by Medaka: `calls_to_draft.bam, calls_to_draft.bam.bai, consensus_probs.hdf`
